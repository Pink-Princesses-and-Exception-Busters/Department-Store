  ...
  const getMenuItems = () => {
    ...
    if (userRole === 'admin' || userRole === 'hq') {
      ...
      return [
        ...
        { path: '/tenants', icon: Building2, label: '입점사 관리' },
        { path: '/product-management', icon: Package, label: '상품 관리' },
        { path: '/customer-service', icon: MessageSquare, label: '고객 서비스' },
      ...
      return [
        ...
        { path: '/customers', icon: Building2, label: '고객 관리' },
        { path: '/dummy-products', icon: Package, label: '더미 상품' },
        ...commonItems.slice(1), // 나머지 공통 메뉴
      ];
    }
  };
